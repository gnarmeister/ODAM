...
def get_bbox_and_orientation(vertices):
    ...
    corners_upper_half = np.concatenate([bbox_2d, np.array([[z_max] * 4]).T], axis=1)
    corners_lower_half = np.concatenate([bbox_2d, np.array([[z_min] * 4]).T], axis=1)
...
def dim2corners(dimensions, orientation):
    ...
    y_corners = [w / 2, -w / 2, -w / 2, w / 2, w / 2, -w / 2, -w / 2, w / 2]
    z_corners = [h / 2, h / 2, h / 2, h / 2, -h / 2, -h / 2, -h / 2, -h / 2]
    corners_3d = np.dot(rot_mat, np.vstack([x_corners, y_corners, z_corners])).T
    return corners_3d
...
def iou_3d(bboxA: "np.ndarray", bboxB: "np.ndarray") -> float:
    ...
    inter_volume = (
        max(0, (x_max - x_min)) * max(0, (y_max - y_min)) * max(0, (z_max - z_min))
    )
...
def box_cxcywh_to_xyxy(bbox):
    ...
    assert (h >= 0).all()
    bbox = np.concatenate(
        [(cx - w / 2.0), (cy - h / 2), (cx + w / 2), (cy + h / 2)], axis=1
    )
    return bbox
...
def box_xyxy_to_cxcywh(bbox):
    ...
    assert (x1 >= x0).all()
    assert (y1 >= y0).all()
    bbox = np.concatenate([(x0 + x1) / 2, (y0 + y1) / 2, (x1 - x0), (y1 - y0)], axis=1)
    return bbox
