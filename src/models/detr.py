...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        self.angle_embed = MLP(hidden_dim, hidden_dim, 60, 3)
    ...
    def postprocess(self, out, img_size, threshold, intr_mat, nms_2d=True):
        ...
        for i in range(b_size):
            ...
            rescale_boxes = (
                boxes * torch.tensor([img_w, img_h, img_w, img_h]).float().cuda()
            )
            rescale_boxes = rescale_boxes.reshape(n_objs, 2, 2)
            rescaled_offset = (
                out["pred_offset"][i] * torch.tensor([img_w, img_h]).float().cuda()
            )
            ...
            keep_objs = self.nms_3d(
                class_, score, t_co, dimension, rescale_boxes, nms_2d
            )
    ...
    def nms_3d(self, class_, scores, t_cos, dimensions, bbox_2d, nms_2d):
        ...
        for i, source_id in enumerate(
            orders
        ):  # start from the detection with highest confidence
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True, **kwargs):
        ...
        target_classes_o = torch.cat(
            [t["objects"][J, 0] for t, (_, J) in zip(targets, indices)]
        ).long()
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        tgt_lengths = torch.as_tensor(
            [len(v["objects"]) for v in targets], device=device
        )
        # Count the number of predictions that are NOT "no-object" (which is the last class)
        card_pred = (pred_logits.argmax(-1) != pred_logits.shape[-1] - 1).sum(1)
        card_pred = torch.sum(
            pred_logits.softmax(-1)[:, :, :-1].max(-1).values > 0.7, dim=1
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        target_boxes = torch.cat(
            [t["objects"][i, 1:5] for t, (_, i) in zip(targets, indices)], dim=0
        )
    ...
    def loss_size(self, outputs, targets, indices, num_boxes, **kwargs):
        assert "pred_size" in outputs
        idx = self._get_src_permutation_idx(indices)
        src_sizes = outputs["pred_size"][idx]
        target_sizes = torch.cat(
            [t["objects"][i, 5:8] for t, (_, i) in zip(targets, indices)], dim=0
        )
        losses = {}
    ...
    def loss_depth(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        target_depths = torch.cat(
            [t["objects"][i, -2] for t, (_, i) in zip(targets, indices)], dim=0
        )
    ...
    #     target_angle = torch.cat(
    #         [t["objects"][i, -1] for t, (_, i) in zip(targets, indices)], dim=0
    #     )
    #     losses = {}
    ...
    def loss_angle(self, outputs, targets, temp_indices, num_boxes, **kwargs):
        assert "pred_angle" in outputs
        categories = kwargs["categories"]
        indices = []
        for i in range(len(temp_indices)):
            indices.append(([], []))
            for k, j in zip(temp_indices[i][0], temp_indices[i][1]):
                category = targets[i]["objects"][j, 0]
                if list(categories.values())[category.int()] == "True":
                    indices[i][0].append(k)
                    indices[i][1].append(j)
                else:
                    num_boxes -= 1
            indices[i] = (
                torch.as_tensor(indices[i][0], dtype=torch.int64),
    ...
    def loss_offset(self, outputs, targets, indices, num_boxes, **kwargs):
        assert "pred_offset" in outputs
        idx = self._get_src_permutation_idx(indices)
        src_offset = outputs["pred_offset"][idx]
        target_offset = torch.cat(
            [t["objects"][i, 8:10] for t, (_, i) in zip(targets, indices)], dim=0
        )
        losses = {}
            ...
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    ...
                    if loss == "labels":
                        ...
                        kwargs = {"log": False, "categories": categories}
                    l_dict = self.get_loss(
                        loss, aux_outputs, targets, obj_indices, num_boxes, **kwargs
                    )
